...
from globals import connected_clients  # 导入全局连接集合
...
class DialogSession:
    ...
    def __init__(self, ws_config: Dict[str, Any], output_audio_format: str = "pcm", audio_file_path: str = "",
        ...
        self.audio_buffer = b''
        self.frontend_queue: asyncio.Queue[bytes] = asyncio.Queue()
    ...
    async def handle_server_response(self, response: Dict[str, Any]) -> None:
        ...
        if response['message_type'] == 'SERVER_ACK' and isinstance(response.get('payload_msg'), bytes):
            ...
            # 语音识别结果
            if event == 451:
                result = (payload_msg.get("results") or [None])[0]
                text = result.get("text", "")

                # 把实时结果推送给前端
                if hasattr(self, 'frontend_queue') and self.frontend_queue and text:
                    await self.frontend_queue.put({"user_text": text})
            
            # AI回复文本结果
            if event == 550:
                text = payload_msg.get("content")
                if hasattr(self, 'frontend_queue') and self.frontend_queue and text:
                    await self.frontend_queue.put({"ai_reply": text})
            ...
            if event == 459 and hasattr(self, 'frontend_queue') and self.frontend_queue:
                await self.frontend_queue.put({"event": "asr_ended"})

            if event == 559 and hasattr(self, 'frontend_queue') and self.frontend_queue:
                await self.frontend_queue.put({"event": "chat_ended"})
